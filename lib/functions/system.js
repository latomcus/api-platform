...
module.exports = {
    ...
    'list apis': function (data_in, callback){
        var filter = ''
        if (data_in.payload.filter){
            filter = data_in.payload.filter
        }

        var app_permissions = config.security.find(o => o.app_key === data_in.app_key).permissions
        ...
        for (var i = 0; i < fns.length; i++){
            ...
            for (var j = 0; j < keys.length; j++){
                ...
                if (name.startsWith('about-->')) {
                    var n = name.replace('about-->','')
                    var p = app_permissions.find(p => p === n)
                    if (filter.length > 0) {
                        if (n.includes(filter)){
                            if (p) { list.push(f[name]()) }
                        }
                    }
                    else {
                        if (p) { list.push(f[name]()) }
