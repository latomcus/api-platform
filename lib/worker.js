...
app.post('/:app_key?/:token?', upload.any(), (req, res) =>{
    ...
    if (!isPermitted(app_key, action)){
        out(res, {code: 'w.0.02', message: config.not_authorized_message})
        delete_files(req.files)
        return
    }

     //at least action must exist
    if (!action) {
        out(res, {code: 'w.0.03', message: 'Missing action'})
        delete_files(req.files)
        return
    }
    ...
    if (req.headers['content-type'].startsWith('multipart/form-data')){
        if (req.files.length === 0){
            out(res, { code: 'w.0.05', message: 'No uploaded files'})
            return
        }

        var fp = [] //array of uploaded files
        for (var i = 0; i < req.files.length; i++){
            log_for_file(req.files[i].filename, 'uploaded')
            fp.push({
...
function delete_files(files){
    console.log('todo: delete_files')
    console.log(files)
}

function log_for_file(file, event, message){
    console.log('todo: log_for_file')
...
function handleRequest(res, data_in){
    ...
    if (typeof fn === 'function'){
        //special case, need to pass list of functions
        if (data_in.action === 'list apis'){
...
function isPermitted(app_key, action){
    var app = config.security.find(a => a.app_key === app_key)
    if (!app){
        return false
    }
    var a = app.permissions.find(a => a === action)
    if (!a){
        return false
    }
    return true
...
process.on('message', message => {
    ...
    if (message.action === 'on_start'){
        if (cluster.worker.id === 1) { //fire up on_start event for first process only
    ...
    if (message.action === 'connect to cache'){
        ...
        .then(function(response){
            var r = response.data
            if (r.message === 'PONG'){
                console.log(`+ ${cluster.worker.id} worker [${cluster.worker.process.pid}] connected to cache. Cache response: ${r.message}`)
            }
            else{
                console.log(`- Error: ${cluster.worker.id} worker [${cluster.worker.process.pid}] connected to cache. Cache error response: ${r.message}`)
            }
        })
        .catch(function(error){
            console.log(`- Error: ${cluster.worker.id} worker [${cluster.worker.process.pid}] ping to cache ${error}`)
        })
    }
...
var unload_functions = function(){
    for (var i = 0; i < functions.length; i++){
        delete require.cache[path.resolve(functions[i].module)]
    }
    functions = []
}
...
function reload_functions(){
    ...
    if (cluster.worker.id === 1) {
        process.send({ action: 'on_start', payload: { }})
    }
}
