...
app.get('/ping', (req, res) =>{
    res.json({ code: 'w.0.02', message: 'PONG'})
})
...
app.post('/:token?', upload.any(), (req, res) =>{
    if (req.headers['content-type'].startsWith('multipart/form-data')){
        ...
        if (!token){
            res.json({ code: 'w.0.05', message: 'Missing token'})
            //todo: delete files
            return
        }

        if (req.files.length === 0){
            return res.json({ code: 'w.0.06', message: 'No uploaded files'})
        }
        ...
        //compose reply
        var r = {code: 'w.0.06', message: 'Uploaded', data: { files: []} }
    ...
    axios.post(config.cache.url + '/find', data_in)
        ...
        .catch(function(error){
            ...
            out(res, { code: 'w.0.04', message: 'Cache system error', error: error })
        })
})
