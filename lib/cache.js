...
app.listen(cache_url.port, () => {
   console.log(`+ Cache: enabled on ${config.cache.url}`)
})
...
var add = function(data_in, data_out, key_type, expiration, minutes){
    ...
    if (cache_array.length < config.cache.max_size){ //check cache overall size
        ...
        switch (key_type) {
            case 'action':
                key_value = data_in.action //+ new Date() //testing full cache
                break
        }
        ...
        cache_array.push({
            data_in: data_in,
            data_out: data_out,
            key_type: key_type, //Options: 'action', or 'token,action', or 'token,action,payload', or 'action,payload'
...
var remove = function(key_type, key_value){
    ...
    if (key_type === 'token,action,payload'){
        cache_array = cache_array.filter((item) =>
            item.data_in.token + item.data_in.action + JSON.stringify(item.data_in.payload) !== key_value)
    }
    
    if (key_type === 'action,payload'){
        cache_array = cache_array.filter((item) =>
            item.data_in.action + JSON.stringify(item.data_in.payload) !== key_value)
    }
}
...
var find = function(data_in){
    ...
    for (var i = 0; i < cache_array.length; i++){
        ...
        if (expires > now){
            ...
            if (c.key_type === 'token,action,payload') {
                if (c.key_value === data_in.action &&
                    c.key_value === data_in.token + data_in.action + JSON.stringify(data_in.payload)) {
                    return update_expiration(c)
                }
            }

            if (c.key_type === 'action,payload') {
                if (c.key_value === data_in.action &&
                    c.key_value === data_in.action + JSON.stringify(data_in.payload)) {
                    return update_expiration(c)
                }
            }
        }
